...
function filterDogOwners(users) {
	return users
		.filter((filterDogOwners) => filterDogOwners.hasDog === true)
		.map((filterDogOwners) => filterDogOwners.name);
}
...
function filterAdults(users) {
	return users
		.filter((filterAdults) => filterAdults.age > 18)
		.map((filterAdults) => filterAdults.name);
}
